...
export abstract class PdoRawConnection implements PdoRawConnectionI {
  ...
  protected abstract doBeginTransaction(
  ...
  protected abstract getStatement(
    sql: string,
    connection: PoolConnection,
  ): Promise<any>;
  ...
  >;
  protected abstract closeStatement(
    statement: any,
    connection: PoolConnection,
  ): Promise<void>;

  protected abstract adaptBindValue(
    value: ValidBindingsSingle,
  ): ValidBindingsSingle;
  ...
  public bindValue(value: ValidBindings): ValidBindings {
    if (Array.isArray(value)) {
      const values: ValidBindingsArray = [];
  ...
  > {
    try {
      ...
      const [sqlProcessed, affectingResults, selectResults, columns] =
  ...
  > {
    try {
      const connection = await this.generateOrReuseConnection();
      const [affectingResults, selectResults, columns] = await this.doQuery(
        connection,
        sql,
      );
  ...
  protected async generateStatement(sql: string): Promise<[string, any]> {
    if (!this.statements.has(sql)) {
      this.statements.set(
        sql,
        await this.getStatement(sql, await this.generateOrReuseConnection()),
      );
    }
