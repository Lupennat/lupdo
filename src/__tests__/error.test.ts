...
describe('Pdo Error', () => {
    ...
    it('Throws Error With Cause', () => {
        ...
        const aggregate = new AggregateErrorFake([new Error('test'), new Error('test2')]);
        pdoError = new PdoError(aggregate);
        expect(pdoError.message).toEqual('test');
        expect(pdoError.cause).toEqual(aggregate);
        const aggregateWithMessage = new AggregateErrorFake([new Error('test'), new Error('test2')], 'Message');
