import { Both, Dictionary, Named, PdoI } from '../../src';
import {
  ATTR_CASE,
...
describe('Fetch Mode', () => {
  ...
  it('Works Fetch Column Case', async () => {
    ...
    expect(
      'camElCol' in (stmt.fetchDictionary().get() as Dictionary),
    ).toBeTruthy();
    expect(
      'camelcol' in (stmt.fetchDictionary().get() as Dictionary),
    ).toBeFalsy();
  ...
  it('Works Last Fetch Is Not Lost After New Statement', async () => {
    ...
    expect(stmt.fetchArray().get()).toEqual([1, 'Edmund', 'Multigender']);
    expect(newStmt.fetchArray().get()).toEqual([
      1,
      'Satterfield Inc',
  ...
  it('Works Fetch Json', async () => {
    const stmt = await pdo.query('SELECT * FROM users limit 5;');
    expect(stmt.fetchDictionary().get()).toEqual({
      id: 1,
      name: 'Edmund',
      gender: 'Multigender',
    });
  ...
  it('Works Fetch Column', async () => {
    ...
    expect(stmt.fetchColumn<number>(4).get).toThrow(
      'Column 4 does not exists.',
    );
  });
  ...
  it('Works Fetch Object Fails Loudly', async () => {
    ...
    expect(() => {
      ...
      'Cannot set property id of #<UserGetter> which has only a getter',
    );
  });
  ...
  it('Works Fetch Closure', async () => {
    ...
    expect(
      stmt
        .fetchClosure<[number, string, string]>(
          (first: number, second: string, third: string) => {
  ...
  it('Works Fetch Named', async () => {
    let stmt = await pdo.query(
      "SELECT * FROM users, companies where users.name = 'Edmund';",
    );
  ...
  it('Works Fetch Pair', async () => {
    ...
    expect(Array.from(pair.keys())).toEqual([
      'Edmund',
      'Kyleigh',
      'Josefa',
      'Cecile',
    ...
    expect(Array.from(pair.values())).toEqual([
      'Multigender',
      'Cis man',
      'Cisgender male',
      'Agender',
  ...
  it('Works Fetch Pair Fails Loudly', async () => {
    ...
    expect(() => {
      ...
      'With fetchPair(), query results must return 2 columns, [3] provided',
    );
    ...
    expect(() => {
      ...
      'With fetchPair(), query results must return 2 columns, [1] provided',
    );
  });
});
