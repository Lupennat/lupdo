import { PdoDriverConstructor, PdoLogger } from '../../src';
import {
  ATTR_CASE,
  ATTR_DEBUG,
  ATTR_DRIVER_NAME,
...
import PdoPreparedStatement from '../../src/support/pdo-prepared-statement';
import PdoStatement from '../../src/support/pdo-statement';
import PdoTransaction from '../../src/support/pdo-transaction';
...
describe('Pdo Api', () => {
  ...
  it('Works Add Driver', () => {
    ...
    expect(
      MockPdo.getAvailableDrivers().includes('fakeTestNotOverride'),
    ).toBeTruthy();
  ...
  it('Works SetLogger', async () => {
    ...
    expect(
      mock.mock.lastCall[1].startsWith(
        'Tarn: resource destroyer threw an exception Error: Error to be logged',
      ),
    ).toBeTruthy();
  });
  ...
  it('Works Disconnect', async () => {
    ...
    await pdo.query('SELECT 1');
    await pdo.disconnect();
    await expect(pdo.query('SELECT 1')).rejects.toThrow(
      'Pdo is Disconnected from pool, please reconnect.',
    );
    await expect(pdo.prepare('SELECT 1')).rejects.toThrow(
      'Pdo is Disconnected from pool, please reconnect.',
    );
    await expect(pdo.exec('SELECT 1')).rejects.toThrow(
      'Pdo is Disconnected from pool, please reconnect.',
    );
    await expect(pdo.beginTransaction()).rejects.toThrow(
      'Pdo is Disconnected from pool, please reconnect.',
    );
    await expect(pdo.getRawPoolConnection()).rejects.toThrow(
      'Pdo is Disconnected from pool, please reconnect.',
    );
  });
