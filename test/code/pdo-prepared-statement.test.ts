...
describe('Pdo Statement', () => {
  ...
  it('Works Statement Bind Typed', async () => {
    ...
    await stmt.execute([new TypedBinding(PARAM_BIGINT, BigInt(1))]);
  ...
  it('Works Statement Bind Array', async () => {
    ...
    await stmt.execute([
      [
        new TypedBinding(PARAM_BIGINT, BigInt(1)),
        new TypedBinding(PARAM_BIGINT, BigInt(2)),
        new TypedBinding(PARAM_BIGINT, BigInt(3)),
      ],
      [
        new NumericTypedBinding(PARAM_DECIMAL, '12345.678', {
          total: 10,
          places: 4,
        }),
        new NumericTypedBinding(PARAM_DECIMAL, '123.456789', {
          total: 10,
          places: 4,
        }),
        new NumericTypedBinding(PARAM_DECIMAL, '1234567890', {
          total: 10,
          places: 4,
        }),
      ],
    ...
    expect(stmt.fetchArray().get()).toEqual([
      '["1","2","3"]',
      '["12345.6780","123.4568","999999.9999"]',
      '["hi","to","friends"]',
    ]);
    ...
    stmt = await pdo.prepare(
      'SELECT ARRAY[:bigints], ARRAY[:decimals], ARRAY[:strings];',
    );
    await stmt.execute({
      bigints: [
        new TypedBinding(PARAM_BIGINT, BigInt(4)),
        new TypedBinding(PARAM_BIGINT, BigInt(5)),
        new TypedBinding(PARAM_BIGINT, BigInt(6)),
      ],
      decimals: [
        new NumericTypedBinding(PARAM_DECIMAL, '12345.678', {
          total: 12,
    ...
    expect(stmt.fetchArray().get()).toEqual([
      '["4","5","6"]',
      '["12345.678","123.457","999999999.999"]',
      '["friends","say","hi"]',
    ]);
  ...
  it('Works Statement Bind null', async () => {
    ...
    expect(stmt.fetchArray().all()).toEqual([
      [20, 'test', '2000-12-26t00:00:00.000z', 1, null],
    ]);
