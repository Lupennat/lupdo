...
> {
  ...
  if (sql.startsWith('select array')) {
    ...
    return [
      {},
      arrays ? [arrays.map((item) => item.slice(1, item.length - 1))] : [],
      [],
    ];
  }
  ...
  const database = transaction
    ? databases[threadId + '_transaction']
    : databases[threadId];
  ...
  if (sql.startsWith('select count(*) as total from users')) {
    return [
      {},
      [[database.users.data.length]],
      [{ name: 'total', table: 'users' }],
    ];
  }
  ...
  if (
    sql.startsWith("select * from users, companies where users.name = 'edmund'")
  ) {
  ...
  if (
    sql.startsWith('select * from users') ||
    sql.startsWith('select id, name, gender from users')
  ) {
    const columns = database.users.columns;
    return [
      {},
  ...
  if (sql.startsWith('delete from users where (`id` =')) {
    const id = sql
      .replace('delete from users where (`id` =', '')
      .replace(')', '')
      .replace(';', '')
      .trim();
    const index = database.users.data.findIndex(
  ...
  if (sql.startsWith('select count(*) from companies')) {
    return [
      {},
      [[database.companies.data.length]],
      [{ name: 'count(*)', table: 'companies' }],
    ];
  }
  ...
  if (sql.startsWith('select * from companies')) {
    ...
    return [
      {},
      (sql.includes('limit 5')
        ? database.companies.data.slice(0, 5)
        : database.companies.data
      ).map((item) => {
        const res: any[] = [];
        for (const column of columns) {
...
export default class FakeDBConnection {
  ...
  constructor(
    public options: { notSafe?: boolean; debug: boolean } = { debug: false },
  ) {
  ...
  public async beginTransaction(): Promise<void> {
    ...
    databases[this.threadId + '_transaction'] = JSON.parse(
      JSON.stringify(databases[this.threadId]),
    );
  ...
  async commit(): Promise<void> {
    ...
    const dbToAssign = JSON.parse(
      JSON.stringify(databases[this.threadId + '_transaction']),
    );
