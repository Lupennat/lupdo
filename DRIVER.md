...
class FakeDriver extends PdoDriver {
    ...
    protected async createConnection(unsecure?: boolean): Promise<ThirdPartyConnectionToDB> {
        ...
        if (unsecure) {
            // remove all overrides, unsecure is true when pdo.getRawDriverConnection() is called
            // connection it's not managed by the pool, so it will never be reused by lupdo core apis
        }
